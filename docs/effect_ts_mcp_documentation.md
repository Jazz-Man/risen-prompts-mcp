# Effect TS @effect/ai - MCP Server Documentation (Оновлена версія)

## Огляд

`@effect/ai` - це бібліотека для створення MCP (Model Context Protocol) серверів з використанням Effect TypeScript. Ця документація базується на реальному сорс-коді та виправляє помилки попередньої версії.

**Ключові відмінності від попередньої документації:**
- `McpServerClient` - це Context.Tag, а не окремий клас з методами
- `McpServer.elicit` вимагає `McpServerClient` в контексті
- Handlers для tools можуть оголошувати залежності через `.addDependency()`
- Правильна структура layer composition для різних транспортів
